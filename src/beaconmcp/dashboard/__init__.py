"""BeaconMCP web dashboard: login + tokens + (optional) Gemini chat.
...
def is_enabled() -> bool:
    ...
    Controlled by ``BEACONMCP_DASHBOARD_ENABLED`` (default on). The Gemini
    key is NOT required here — without it, only ``/app/login`` and
    ``/app/tokens`` are meaningful, and ``/app/chat`` redirects to
    tokens.
    """
    flag = os.environ.get("BEACONMCP_DASHBOARD_ENABLED", "true").strip().lower()
