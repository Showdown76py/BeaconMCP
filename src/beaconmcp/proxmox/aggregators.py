...
def _collect_node_summaries(client: ProxmoxClient) -> list[dict[str, Any]]:
    ...
    for node_name in client.configured_nodes:
        ...
        if isinstance(data, dict) and "error" in data:
            out.append({"name": node_name, "status": "unreachable", "error": data["error"]})
            continue
        if not isinstance(data, list):
            out.append({"name": node_name, "status": "unknown"})
            continue
        for node in data:
...
def _collect_vm_summaries(
    ...
    """
    nodes = target_nodes or client.configured_nodes
...
def _collect_storage_summaries(client: ProxmoxClient) -> list[dict[str, Any]]:
    ...
    for n in client.configured_nodes:
        ...
        if isinstance(data, dict) and "error" in data:
            rows.append({"node": n, "error": data["error"]})
            continue
        if not isinstance(data, list):
            continue
        for s in data:
...
def register_aggregator_tools(
    ...
    async def cluster_health(node: str = "") -> dict[str, Any]:
        ...
        for n in target_nodes:
            status = proxmox_client.get(n, f"nodes/{n}/status")
            if isinstance(status, dict) and "error" in status:
                results.append({"node": n, "error": status["error"]})
                continue
            entry: dict[str, Any] = {
                "node": n,
            ...
            bmc = await _bmc_summary(bmc_registry, config, n)
            if bmc is not None:
                entry["bmc"] = bmc
            entry["recent_errors"] = _recent_errors(proxmox_client, n, limit=20)
    ...
    def vm_bulk_action(
        ...
        for vmid in vmids:
            ...
            if not location:
                results.append({"vmid": vmid, "error": "not found"})
                continue
            n, vm_type = location
            ...
            if isinstance(resp, dict) and "error" in resp:
                results.append({"vmid": vmid, "node": n, "error": resp["error"]})
            else:
                upid = resp if isinstance(resp, str) else (
                    resp.get("upid") if isinstance(resp, dict) else None
                )
                results.append({"vmid": vmid, "node": n, "type": vm_type, "upid": upid})
