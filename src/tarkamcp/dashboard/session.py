...
with AES-256-GCM, keyed by the env var ``TARKAMCP_SESSION_KEY``.
"""
...
def load_session_key() -> bytes:
    ...
    """
    raw = os.environ.get("TARKAMCP_SESSION_KEY", "").strip()
    if not raw:
        raise RuntimeError(
            "TARKAMCP_SESSION_KEY is required to enable the dashboard. "
            "Run `openssl rand -base64 32` and set it in /opt/tarkamcp/.env."
        )
    try:
        key = base64.b64decode(raw, validate=True)
    except Exception as e:  # noqa: BLE001
        raise RuntimeError(
            f"TARKAMCP_SESSION_KEY is not valid base64: {e}"
        ) from e
    if len(key) != 32:
        raise RuntimeError(
            f"TARKAMCP_SESSION_KEY must decode to 32 bytes, got {len(key)}."
        )
    return key
