...
def test_single_hp_ilo_device() -> None:
    dev = BMCDevice(id="rack1-ilo", type="hp_ilo", host="10.0.0.20", user="admin", password="pw")
...
def test_multiple_mixed_devices() -> None:
    cfg = _make_config(
        [
            BMCDevice(id="ilo", type="hp_ilo", host="10.0.0.20", user="a", password="x"),
            BMCDevice(id="ipmi", type="ipmi", host="10.0.0.21", user="a", password="y"),
            BMCDevice(id="dell", type="idrac", host="10.0.0.22", user="a", password="z"),
            BMCDevice(id="smci", type="supermicro", host="10.0.0.23", user="a", password="w"),
        ]
    )
...
def test_unknown_type_raises_at_startup() -> None:
    cfg = _make_config(
        [BMCDevice(id="x", type="nope", host="10.0.0.20", user="a", password="b")]
    )
